...
def registrar_pago(
    ...
    credito = db.query(Credito).filter(_uuid_eq(Credito.id, data.credito_id)).first()
    if not credito:
        raise HTTPException(status_code=404, detail="Crédito no encontrado")

    # Verificar que el crédito pertenece al negocio
    if _uuid_eq(Credito.negocio_id, negocio_id) != True:
        raise HTTPException(
            status_code=400,
            detail="El crédito no pertenece a este negocio",
        )
